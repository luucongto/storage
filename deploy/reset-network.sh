...
for KEY in "${!SERVERS[@]}"; do
  SERVER=${SERVERS[$KEY]}
  echo "Stopping onematrixd service on ${KEY}..."
  ssh ubuntu@${SERVER} "sudo service onematrixd stop && onematrixd comet unsafe-reset-all --home /home/ubuntu/.onematrix"

done

for KEY in "${!SERVERS[@]}"; do
  SERVER=${SERVERS[$KEY]}
  echo "Starting onematrixd service on ${KEY}..."
  ssh ubuntu@${SERVER} "sudo service onematrixd start"
done
